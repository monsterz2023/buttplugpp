...
ButtplugClient::ButtplugClient(asio::io_context &ioc, std::string host,
                               std::string port)
    : resolver_(ioc), ws_(ioc), host_(std::move(host)), port_(std::move(port)) {
}
...
void ButtplugClient::connect(Callback connected_cb) {
  resolver_.async_resolve(
      host_, port_,
      [this, connected_cb](boost::system::error_code ec,
...
void ButtplugClient::request_device_list(
    std::function<void(std::vector<Device>)> cb) {
  auto id = Util::random_message_id();
  json req = {{"RequestDeviceList", {{"Id", id}}}};
...
void ButtplugClient::send_linear_cmd(std::uint32_t device_idx,
                                     std::uint32_t position,
                                     std::uint32_t duration) {
  auto id = Util::random_message_id();
  auto p = Util::clamp<std::uint32_t>(position, 0, 100);
  json vectors = json::array({});
  json v = {{"Index", 0}, {"Duration", duration}, {"Position", p / 100.0}};
  vectors.push_back(v);

  json msg_body = {
      {"Id", id}, {"DeviceIndex", device_idx}, {"Vectors", vectors}};
  json req = {{"LinearCmd", msg_body}};
...
void ButtplugClient::register_client() {
  auto id = Util::random_message_id();
  json message_body = {
      {"Id", id}, {"ClientName", "Buttplug C++ Client"}, {"MessageVersion", 3}};
  json req = {{"RequestServerInfo", message_body}};
...
void ButtplugClient::do_write() {
  ...
  ws_.async_write(asio::buffer(request),
  ...
  for (auto &ele : j) {
    for (auto &kv : ele.items()) {
      if (kv.value().contains("Id")) {
        int id = kv.value()["Id"];
        if (id == 0) {
          std::cout << "Server event: " << kv.key() << "|" << ele.dump()
                    << std::endl;
        } else {
